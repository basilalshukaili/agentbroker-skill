...
**Fit:** AgentBroker supports x402 for write-tool billing ($9/90d via Polar); sanctions + company verification are high-value compliance services for agent workflows.  
...
```markdown
- [Agent Broker](https://hatchloop.dev/agent-broker/) - Live GLEIF+SEC company verification, OFAC/EU/UN/UK+40 sanctions screening, and cross-border trade-restriction mapping as MCP calls. 11 read tools free (no key, no x402). Write tools (SMS, booking, voice calls to SMBs) billed at $9/90d via Polar. MCP: `https://hatchloop.dev/mcp/agent-broker`. ([Skill/Install](https://github.com/basilalshukaili/agentbroker-skill) | [MCP](https://hatchloop.dev/mcp/agent-broker))
```
